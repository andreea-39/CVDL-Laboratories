# -*- coding: utf-8 -*-
"""Computer Vision and Deep Learning - Laboratory 3 [students].ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1vgZNqOtttMDVLTev1ClIorTcff_YPt80
...
#!wget https://www.researchgate.net/profile/Paul-Rodriguez-5/publication/265988128/figure/fig1/AS:459614656438273@1486592081799/Input-test-images-a-Cameraman-grayscale-b-grayscale-Lena-and-c-color-Lena-All_Q640.jpg -O cameraman.jpg
...
color = color.resize((200, 200), )
color = np.asarray(color)

# X contains a single image sample
X = np.expand_dims(color, axis=0)

############################################################
# MEAN FILTER
############################################################

bias = np.asarray([0, 0, 0])
bias = bias.reshape((1, 1, 1, 3))

mean_filter_3 = np.ones(shape=(3, 3, 3, 1), dtype=np.float32)
mean_filter_3 = mean_filter_3/9

mean_filter_9 = np.ones(shape=(9, 9, 3, 1), dtype=np.float32)
mean_filter_9 = mean_filter_9/81
...
plt.title('Original image')
plt.subplot(1, 2, 2)
plt.imshow(mean_3x3[0].squeeze(), cmap='gray')
plt.title('mean filter 3x3')
...
plt.title('Original image')
plt.subplot(1, 2, 2)
plt.imshow(mean_9x9[0].squeeze(), cmap='gray')
plt.title('mean filter 9x9')
...
gaussian_filter = gaussian_filter/16.0
gaussian_filter = gaussian_filter[:, :, None] + np.zeros((1,3))
gaussian_filter = gaussian_filter.reshape(3, 3, 3, 1)
gaussian_smoothed = convolution(X, gaussian_filter, bias, pad=0, stride=1)
...
plt.title('Original image')
plt.subplot(1, 2, 2)
print(gaussian_smoothed[0, 0, 0, 0])
plt.imshow(gaussian_smoothed[0].squeeze(), cmap='gray')
plt.title('Gaussian filtered')
