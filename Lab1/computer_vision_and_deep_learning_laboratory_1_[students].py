...
#a)
tests = np.delete(statistics, 3, 0)
testsSum = np.sum(tests, 0)
print('The total number of tests performed each day is: ', testsSum)

#b)
positiveTests = np.array(statistics)[0]
percentages = np.divide(positiveTests, testsSum)
percentages = np.multiply(percentages, 100)
print('The percentage of daily positive tests and the percentage of daily inconclusive tests:', percentages)

#c)
index = np.argmax(statistics[3], axis=0)
print('The day in which the maximum number of deaths occured is:', days[index])

#d)
positiveTests = positiveTests[:5]
deaths = np.array(statistics)[3][:5]


illogicSum = np.add(positiveTests, deaths)
print('The sum of daily deaths and positive test results for each working day is:', illogicSum)

#e)
print('The sum of daily deaths and positive test results for each working day is:', np.sum(statistics, 1))
"""Expected output:
...
## YOUR CODE HERE
hist_r, bins_r = np.histogram(img_resize_prop[0], bins=255)
hist_g, bins_g = np.histogram(img_resize_prop[1], bins=255)
hist_b, bins_b = np.histogram(img_resize_prop[2], bins=255)

plt.bar(np.arange(255), hist_r, color='red')
plt.bar(np.arange(255), hist_g, color='green')
plt.bar(np.arange(255), hist_b, color='blue')
...
## YOUR CODE HERE
hist, bins = np.histogram(img_l1, bins=255)# [0-255], 256

print('The histogram is:\n', hist)

plt.bar(np.arange(255), hist, color='cornflowerblue')
plt.title('histogram of the grayscale image')s
...
## YOUR CODE HERE
hist, bins = np.histogram(img_l2, bins=255)# [0-255], 256
...
im = (im.astype(np.float32) - np.array([103.939, 116.779, 123.68]).reshape((1,1,3)))
to_predict = im.reshape((1, 224, 224, 3))
...
fool = im.copy()
fool[:, :, 2], fool[:, :, 0] = fool[:, :, 0], fool[:, :, 2]
to_predict = fool.reshape((1, 224, 224, 3))
preds = model.predict(to_predict + 50)
